...
def get_multiinput_transformer(
    ...
    genome_input_r = get_seq_r(genome_input,name="genome_r")
    ...
    genome_layer_r = get_layer(
        inbound_layer=genome_input_r,
        filters=filters*4,
        kernel_size=input_kernel_size,
        activation="linear",
        padding=padding,
        ...
        kernel_initializer=KERNEL_INITIALIZER,
        n=1,
        name="first_conv_kernel_r",
        skip_batch_norm=True,
    )

    genome_layer=tf.keras.layers.Concatenate(axis=-1)([genome_layer,genome_layer_r])
