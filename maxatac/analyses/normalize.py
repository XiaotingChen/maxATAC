...
def run_normalization(args):
    ...
    chromosome_length_dictionary = build_chrom_sizes_dict(
    ...
    if args.max:
        ...
        min_value, max_value, median, mean_value, std_value = get_genomic_stats(
            bigwig_path=args.signal,
        ...
        logging.info(
            "Sample Statistics"
            + "\n  Genomic minimum value: "
            + str(min_value)
            + "\n  Genomic max value: "
            + str(max_value)
            + "\n  Genomic median (non-zero): "
            + str(median)
            + "\n  Genomic mean: "
            + str(mean_value)
            + "\n  Genomic standard deviation: "
            + str(std_value)
        )
    ...
    with pyBigWig.open(args.signal) as input_bw, pyBigWig.open(
        output_filename, "w"
    ) as output_bw:
        header = [
            (x, chromosome_length_dictionary[x]) for x in sorted(args.chromosomes)
        ]
        ...
        for chrom_name, chrom_length in header:
            chr_vals = np.nan_to_num(
                input_bw.values(chrom_name, 0, chrom_length, numpy=True)
            )

            if args.method == "min-max":
                normalized_signal = minmax_normalize_array(
                    chr_vals, min_value, max_value, clip=args.clip
                )

            elif args.method == "zscore":
                normalized_signal = zscore_normalize_array(
                    chr_vals, mean=mean_value, std_dev=std_value
                )
            ...
            blacklist_mask = chromosome_blacklist_mask(
                args.blacklist_bw, chrom_name, chromosome_length_dictionary[chrom_name]
            )
