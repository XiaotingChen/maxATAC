...
from maxatac.utilities.threshold_tools import (
    import_blacklist_mask,
    import_GoldStandard_array,
    calculate_AUC_per_rank,
)
...
def extract_pred_gs_bw(
    bigwig_file, training_data_dict, chrom_name, chrom_length, bin_count
):
    start = time.time()
    ...
    goldstandard_array = import_GoldStandard_array(
        training_data_dict[bigwig_file], chrom_name, chrom_length, bin_count
    )
...
def run_thresholding(args):
    ...
    training_data_dict = pd.Series(
        meta_DF["Binding_File"].values, index=meta_DF["Prediction"]
    ).to_dict()
    ...
    for chrom_name, chrom_length in chromosome_sizes_dictionary.items():
        bin_count = int(
            int(chrom_length) / int(args.bin_size)
        )  # need to floor the number

        blacklist_mask = import_blacklist_mask(
            args.blacklist_bw, chrom_name, chrom_length, bin_count
        )
    ...
    precision, recall, thresholds = precision_recall_curve(
        DF["GoldStandard"][blacklist], DF["Prediction"][blacklist]
    )
    ...
    df = pd.DataFrame(
        {
            "Precision": precision,
            "Recall": recall,
            "Threshold": np.insert(thresholds, 0, 0),
        }
    )
    ...
    PR_CURVE_DF = pd.DataFrame(
        {"Precision": P, "Recall": R, "Threshold": np.insert(thresholds, 0, 0)}
    )
    ...
    BED_df_bedtool = pybedtools.BedTool().window_maker(
        g=args.chrom_sizes, w=args.bin_size
    )
    ...
    # Random Precision
    PR_CURVE_DF["Random_Precision"] = (
        PR_CURVE_DF["Total_Avg_GoldStandard_Bins"] / rand_bins
    )

    logging.info("Calculate log2FC for each threshold")

    # Log2FC
    PR_CURVE_DF["log2FC_Precision_Random_Precision"] = np.log2(
        PR_CURVE_DF["Precision"] / PR_CURVE_DF["Random_Precision"]
    )

    logging.info("Calculate F1 Score for each threshold")

    # F1 Score
    PR_CURVE_DF["F1_Score"] = (
        2
        * (PR_CURVE_DF["Precision"] * PR_CURVE_DF["Recall"])
        / (PR_CURVE_DF["Precision"] + PR_CURVE_DF["Recall"])
    )
