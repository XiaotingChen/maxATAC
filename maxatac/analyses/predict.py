...
def run_prediction(args):
    ...
    if args.TF:
        args.model = glob.glob(
            os.path.join(args.DATA_PATH, "models", args.TF, args.TF + "*.h5")
        )[0]

        args.cutoff_file = glob.glob(
            os.path.join(args.DATA_PATH, "models", args.TF, args.TF + "*.tsv")
        )[0]
    ...
    chrom_list = list(
        set(args.chromosomes).intersection(set(regions_pool["chr"].unique()))
    )

    logging.info(
        "Prediction Parameters \n"
        + f"Output filename: {outfile_name_bigwig} \n"
        + f"ATAC-seq signal: {args.signal} \n"
