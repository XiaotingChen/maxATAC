...
def export_model_structure(
    model, file_location, suffix="_model_structure", ext=".png", skip_tags="_{epoch}"
):
    plot_model(
        model=model,
        show_shapes=True,
        ...
        expand_nested=True,
        show_dtype=True,
        to_file=replace_extension(remove_tags(file_location, skip_tags), suffix + ext),
    )


def export_binary_metrics(
    history,
    tf,
    RR,
    ARC,
    ...
    fig, ((ax1, ax2)) = plt.subplots(
        1, 2, sharex=False, sharey=False, figsize=(18, 6), dpi=200
    )
    fig.suptitle(
        "Loss and Dice Coefficient with random ratio set at "
        + str(RR)
        + "  for "
        + tf
        + " using the "
    ...
    fig.savefig(
        replace_extension(remove_tags(file_location, skip_tags), suffix + ext),
        bbox_inches="tight",
    )


def export_loss_mse_coeff(
    history,
    tf,
    TCL,
    RR,
    ...
    fig, ([ax1, ax2, ax3, ax4], [ax5, ax6, ax7, ax8]) = plt.subplots(
        2, 4, sharex=False, sharey=False, figsize=(24, 12)
    )
    fig.suptitle(
        "Training and Validation: Loss, Mean Squared Error and Coefficiennt of Determination for PCPC training on "
        + TCL
        + "\n"
        + " with random ratio set at "
        + str(RR)
        + "  for "
        + tf
        + " "
...
def export_prc(
    ...
    plt.savefig(replace_extension(file_location, suffix + ext), bbox_inches="tight")
